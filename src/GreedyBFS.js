let queue = [];
export default function Greedy(grid, startNode, endNode) {
  ...
  while (queue.length > 0) {
    ...
    if (currentNode.node.i === endNode.i && currentNode.node.j === endNode.j) {
      // Goal reached, reconstruct path and break
      let current = endNode;
    ...
    manhattan=manhattan+neighbor.isWeight;
    
      queue.push({ node: neighbor, heuristic: manhattan });
      

      // Store the predecessor for each visited node
